...
## Cloudflare Dev Mode Uses Miniflare

In Nitro v3, Cloudflare dev mode now runs your code inside a [Miniflare](https://miniflare.dev/) (workerd) runtime instead of using `getPlatformProxy`. This provides a closer-to-production environment.

During Cloudflare dev emulation:
- Bindings are automatically read from `cloudflare.wrangler` within `nitro.config.ts` in addition to `wrangler.json`/`wrangler.jsonc`/`wrangler.toml`.
- The `CLOUDFLARE_ENV` environment variable allows you to select the Wrangler environment.
- Per-binding remote mode (`remote: true`) is supported.
